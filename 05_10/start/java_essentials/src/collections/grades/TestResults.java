...
public class TestResults {
    ...
    public static void main(String[] args) {
        Map<String, Integer> gradesFinal = getOriginalGrades();
        Map<String, Integer> makeupGrades = getMakeUpGrades();
        gradesFinal.forEach((student, originalGrade) -> {
            Integer makeupGrade = makeupGrades.get(student);
            if (makeupGrade > originalGrade) {
                gradesFinal.put(student, makeupGrade);
            }
            System.out.println("Student: " + student + " final grade: " + gradesFinal.get(student));
        });
